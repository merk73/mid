...
function createClientCardLevels(record, options = {}) {
  const threatValue = clientCardFieldValue(record, ["Уровень угрозы"]) || (record?.threatLevel ? `T${record.threatLevel}` : "");
  const accessValue = clientCardFieldValue(record, ["Уровень доступа", "Осведомленность клиента"]) || (record?.accessLevel ? `D${record.accessLevel}` : "");
  const levels = [
